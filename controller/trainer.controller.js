...
const getProfile = async (req, res, next) => {
  try {
    ...
    if (foundTrainer) {
      res.render("trainer/trainer-profile", {
        _id,
        username,
        email,
        profile,
...
const postProfile = async (req, res, next) => {
  try {
    ...
    const foundTrainer = await Trainer.findOne({ user: idUser }).populate({
      path: "user",
    });
    console.log(foundTrainer);
...
const postCourses = async (req, res, next) => {
  try {
    ...
    if (!title) {
      return res
        .status(400)
        .render("auth/signup", { errorMessage: "The title fiel is required" });
    }
    if (!institution) {
      return res
        .status(400)
        .render("auth/signup", {
          errorMessage: "The institution fiel is required",
        });
    }
    ...
    if (req.file) {
      ...
      if (createCourse) {
        ...
        return res
          .status(400)
          .render("trainer/trainer-profile", {
            errorMessage: "could not add the course",
          });
      }
    } else {
      return res
